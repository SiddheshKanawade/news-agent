...
from prazo.utils.chat_models import EmbeddingModel, ChatModel
from prazo.core.logger import logger


def get_embeddings(combined_articles: List[str]) -> List[List[float]]:
    embedding_model = EmbeddingModel(provider="openai", model_name="text-embedding-3-small").get_model()
...
def merge_two_articles(article1: NewsItem, article2: NewsItem) -> NewsItem:
    ...
    **News Item 2:**
    ...
    try:
        ...
        return NewsItem(
            title=merged_title or f"{article1.title} / {article2.title}"[:100],  # Fallback
            summary=merged_summary or f"{article1.summary}\n\n{article2.summary}",  # Fallback
            sources=all_sources,
            published_date=(
                max(d for d in [article1.published_date, article2.published_date] if d is not None)
                if any([article1.published_date, article2.published_date])
                else None
            ),
            topic=article1.topic,
            groups=article1.groups,
            tool_source=','.join([article1.tool_source, article2.tool_source]) if article1.tool_source and article2.tool_source else article1.tool_source or article2.tool_source
        )
...
def merge_similar_articles(articles: List[NewsItem], similar_articles: List[List[int]]) -> List[NewsItem]:
    """
    ...
    for similar_article_indices in similar_articles:
        ...
        if len(similar_article_indices) > 1:
            logger.info(f"Merging {len(similar_article_indices)} similar articles: {similar_article_indices}")
...
def compare_embeddings(embeddings: List[List[float]]) -> List[List[float]]:
    ...
    logger.info(f"Found {len(similar_articles)} unique article groups (threshold: 0.90)")
...
def deduplicate(articles: List[NewsItem]) -> List[NewsItem]:
    ...
    # Combine articles 
    combined_articles: List[str] = [combine_article(article) for article in articles]
    ...
    logger.info(f"Deduplication complete: {len(articles)} → {len(deduplicated_articles)} articles")
    return deduplicated_articles